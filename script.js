...
document.getElementById("rentaForm").addEventListener("submit", function(e) {
  ...
  resultado.innerHTML = debeDeclarar
    ? `✅ <strong>OBLIGADO A DECLARAR</strong><br>Motivos: ${razones.join(", ")}<br><strong>Fecha límite:</strong> ${fechasPresentacion[ultimosDos]}`
...
document.getElementById("btnPDF").addEventListener("click", function() {
  ...
  if (!data) return alert("Primero valida el formulario");
  ...
  doc.setFontSize(12);
  doc.text(`NIT: ${data.nit}`, 20, 45);
  doc.text(`Resultado: ${data.debeDeclarar ? "OBLIGADO A DECLARAR" : "NO OBLIGADO"}`, 20, 55);

  if (data.debeDeclarar) {
    doc.text(`Fecha límite: ${fechasPresentacion[data.ultimosDos]}`, 20, 65);
    doc.text("Motivos:", 20, 80);
    data.razones.forEach((r, i) => doc.text(`- ${r}`, 25, 90 + i * 8));
  }
